...
class PostService extends BaseService implements PostServiceInterface
{
    ...
    public function create($request, $languageId){
        ...
        try{
            ...
            if($post->id > 0){
                ...
                $post = $this->postRepository->getPostById($post->id, $languageId);

                $customers = CustomerPostCatalogue::where('post_catalogue_id', $post->post_catalogue_id)->get();

                if(count($customers)){

                    foreach($customers as $key => $customer){

                        $customer = Customer::where('id', $customer->customer_id)->first();

                        SendPostEmailJob::dispatch($customer, $post);
    ...
    private function updateTag($post, $request){

        $payload['tag'] = $request->input('tag');

        $temp = [];

        foreach($payload['tag'] as $key => $val){
        ...
        $post->tag_post()->detach();

        $post->tag_post()->sync($temp);
    }

    private function createNotification($post){

        $post_id = $post->id;

        $posts = $this->postRepository->findById($post_id);

        $post_catalogue_id = $posts->post_catalogue_id;

        $temp = CustomerPostCatalogue::where('post_catalogue_id', $post_catalogue_id)->count();

        if($temp == 1){

            $notification = $post->notification_post()->create(
                [ 'post_id' => $post_id ]
            );
    
            $notification_id = $notification->id;
            
            $customers = (Customer::where('alert',1)->get('id'))->toArray();
            ...
            DB::table('notification_customer')->insert($temp);
