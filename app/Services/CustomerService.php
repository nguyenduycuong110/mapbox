...
class CustomerService extends BaseService implements CustomerServiceInterface 
{
    ...
    public function create($request){
        DB::beginTransaction();
        try{
            $payload = $request->except(['_token','send','re_password', 'confirm']);
    ...
    public function update($id, $request){
        ...
        try{
            ...
            if(isset($payload['code_premium'])){

                $code = $payload['code_premium'];

                $codeRecord = Code::where('code', $code)->first();

                if($codeRecord){

                    $payload['customer_catalogue_id'] = 5;

                    $temp = ['publish' => 0];

                    $this->codeRepository->update($codeRecord->id, $temp);
