                    ...
                    @if(__('messages.open_hours'))
                        ...
                        <div class="input-select">
                            @foreach (__('messages.open_hours') as $key => $val)
                                ...
                                <input 
                                    ...
                                    type="text" 
                                    value="{{ $val }}" 
                                    class="op-hour {{ isset($homestay->open_hours) && $homestay->open_hours == $key ? 'active' : '' }}"
                                    readonly
                                >
                            @endforeach
                        </div>
                        <select name="open_hours" id="open_hours" class="form-control setupSelect2">
                            ...
                            @foreach (__('messages.open_hours') as $key => $val)
                                <option 
                                    value="{{ $key }}"
                                    {{ ($open_hours == $key || (isset($homestay) && $homestay->open_hours == $key)) ? 'selected' : '' }}
                                >
                                    {{ $val }}
                                </option>
                            @endforeach
                    ...
                    @if(__('messages.total_rooms'))
                        ...
                        <div class="input-select">
                            @foreach (__('messages.total_rooms') as $key => $val)
                                ...
                                <input
                                    ...
                                    type="text" 
                                    value="{{ $val }}" 
                                    class="total-rooms {{ isset($homestay->total_rooms) && $homestay->total_rooms == $key ? 'active' : '' }}" 
                                    readonly
                                >
                            @endforeach
                        </div>
                        <select name="total_rooms" id="total_rooms" class="form-control setupSelect2">
                            ...
                            @foreach (__('messages.total_rooms') as $key => $val)
                                <option 
                                    value="{{ $key }}"
                                    {{ ($total_rooms == $key || (isset($homestay) && $homestay->total_rooms == $key)) ? 'selected' : '' }}
                                >
                                    {{ $val }}
                                </option>
                            @endforeach
                    ...
                    @if(__('messages.current_guests'))
                        ...
                        <div class="input-select">
                            @foreach (__('messages.current_guests') as $key => $val)
                                ...
                                <input 
                                    data-value="{{ $key }}" 
                                    type="text" value="{{ $val }}" 
                                    class="current_guest {{ isset($homestay->current_guests) && $homestay->current_guests == $key ? 'active' : '' }}" 
                                    readonly
                                >
                            @endforeach
                        </div>
                        <select name="current_guests" id="current_guests" class="form-control setupSelect2">
                            ...
                            @foreach (__('messages.current_guests') as $key => $val)
                                <option 
                                    value="{{ $key }}"
                                    {{ ($current_guests == $key || (isset($homestay) && $homestay->current_guests == $key)) ? 'selected' : '' }}
                                >
                                    {{ $val }}
                                </option>
                            @endforeach
